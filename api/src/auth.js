...
const ORGANISATION_ALREADY_REGISTERED = "ORGANISATION_ALREADY_REGISTERED";
...
class Auth {
  ...
  async signup(req, res) {
    try {
      const { password, username, organisation, role, avatar } = req.body;

      if (password && !validatePassword(password)) return res.status(200).send({ ok: false, user: null, code: PASSWORD_NOT_VALIDATED });
      const organisationExisting = await this.model.findOne({ organisation });
      if (!organisationExisting) {
        const user = await this.model.create({ name: username, organisation, password, role, avatar });
        ...
        return res.status(409).send({ ok: false, code: ORGANISATION_ALREADY_REGISTERED });
      }
    } catch (error) {
      console.log("e", error);
