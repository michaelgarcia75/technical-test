...
const UserCard = ({ hit, projects }) => {
  ...
  return (
    <div
      onClick={user.role === "ADMIN" ? () => history.push(`/user/${hit._id}`) : undefined}
      ...
      <div className="flex flex-col flex-1 justify-between">
        <div className="flex flex-col items-center text-center my-4 space-y-1">
          ...
          {hit.role === "ADMIN" ? (
            <p className="font-semibold text-lg" style={{ color: "red" }}>
              {hit.role}
            </p>
          ) : (
            <p className="font-semibold text-lg">{hit.role}</p>
          )}
        </div>
      </div>
    </div>
